...
//	Jump in data access - not vectorizable
	...
	for (int nl = 0; nl < 200*(ntimes/(LEN2)); nl++) {
		for (int i = 0; i < LEN2; i++) {			
			for (int j = 0; j < i; j++) {
				//**loop not vectorized: unsafe dependent memory operations in loop (aa[j][i])
...
//	linear dependence testing
	...
	for (int nl = 0; nl < ntimes*10; nl++) {
		for (int i = 0; i < LEN - 5; i += 5) {
			...
			//used outside the loop(occurred at last a[i])
			a[i] = a[i + 1] * a[i];
...
int s124()
{
//**tsc.c:1293:1: remark: loop not vectorized: control flow cannot be substituted for a select
